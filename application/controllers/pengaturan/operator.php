...
class operator extends ApplicationBase {
    ...
    public function index() {
        ...
        $user_st = empty($search['user_st']) ? '%' : $search['user_st'];
        $this->smarty->assign("search", $search);
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("pengaturan/operator/index/");
        $config['total_rows'] = $this->m_operator->get_total_operator(array($operator_name));
    ...
    public function add() {
        ...
        // load roles
        $this->smarty->assign("rs_roles", $this->m_operator->get_all_roles_by_portal(2));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // untuk hak akses
    ...
    public function add_process() {
        ...
        $this->tnotification->set_rules('operator_jabatan', 'Jabatan', 'trim|required|maxlength[50]');
        ...
        // check email
        $email = trim($this->input->post('operator_email'));
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_operator->insert_user($params)) {
                ...
                $params = array(
                    ...
                    $this->input->post('operator_gender'), $this->input->post('operator_jabatan'),
                    $user_id
                );
                // insert users
                if ($this->m_operator->insert_operator($params)) {
                    ...
                    if (!empty($roles)) {
                        // delete
                        $this->m_operator->delete_user_role(array($user_id, 2));
                        // insert
    ...
    public function edit($user_id = "") {
        ...
        // load style ui
        $this->smarty->load_style("jquery-ui/jquery-ui.css");
    ...
    public function account($user_id = "") {
        ...
        $this->smarty->assign("result", $result);
        // roles selected
        $this->smarty->assign("roles_selected", $this->m_operator->get_all_roles_by_user(array($user_id, 2)));
        // notification
        $this->tnotification->display_notification();
        $this->tnotification->display_last_field();
        // untuk hak akses
    ...
    public function process_update_account() {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if ($this->m_operator->update_data_account($params)) {
                ...
                if (!empty($roles)) {
                    // delete
                    $this->m_operator->delete_user_role(array($this->input->post('user_id'), 2));
                    // insert
                    foreach ($roles as $role_id) {
